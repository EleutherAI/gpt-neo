...
parser.add_argument("--chunk_size", type=int, default=2049, help="How big a chunk should be in chunk mode")
...
def create_tfrecords(params, write_remainder=True, write_every_n_files=10, save_checkpoints=False, resume_from_checkpoint=False, display_pbar=False):
    ...
    for f in files:
        for tokenized_files in archive_to_tokens(f, enc, args):
            ...
            if len(data_to_prepend) >= args.chunk_size + 1:
...
if __name__ == "__main__":
    ...
    logging.warning(f"Make sure that chunk size ({args.chunk_size}) = your model's context size * + 1 * - \
