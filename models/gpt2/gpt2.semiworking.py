...
def scale_norm(x, scope, *, axis=sentinel, epsilon=1e-8, params=None):
    if axis is sentinel:
        axis = x.shape[-1]
...
def layer_norm(x, scope, *, axis=sentinel, epsilon=1e-8, params=None):
...
def linear(x, scope, nf, *, w_init_stdev=0.02, params=None, scale=False, bias=True):
    ...
    with tf.variable_scope('conv1d_main'):
        c = mtf.layers.dense(x, new_dims=[nf], reduced_dims=[x.shape[-1]], name=scope, use_bias=bias,
...
def attn(x, scope, n_state, *, layer_num, past, params, bias, memory_length_dim, train=False, context=None):
    ...
    with tf.variable_scope(scope):
        ...
        hidden_dims = mtf.Dimension('dim_heads', dim_heads.size * dim_kv.size)
        dim_intermediate = mtf.Dimension('qkv_dimension', hidden_dims.size * 3)
        qkv = linear(x, 'to_qkv', dim_intermediate, bias = False, params = params)

        q, k, v = mtf.split(qkv, dim_intermediate, 3)
        q, k, v = map(lambda t: mtf.reshape(t, [dim_batch, dim_seq, dim_heads, dim_kv]), (q, k, v))
        q, k, v = map(lambda t: mtf.transpose(t, [dim_batch, dim_heads, dim_seq, dim_kv]), (q, k, v))
        ...
        with tf.variable_scope('attention'):
            if  attention_type == "local":
                ...
                out = mtf_transformer.attention.local_attention_1d(
                    q, k, v,
                ...
                k = mtf.replace_dimensions(k, k.shape[2], memory_length_dim)
                v = mtf.replace_dimensions(v, v.shape[2], memory_length_dim)
                ...
                dots = mtf.layers.us_einsum([q, k], [dim_batch, dim_heads, dim_seq, memory_length_dim])

                i = mtf.range(mesh, dim_seq, tf.int32)
                j = mtf.range(mesh, memory_length_dim, tf.int32)
                i, j = map(lambda t: mtf.broadcast(t, [dim_seq, memory_length_dim]), (i, j))
                mask = mtf.less(i + memory_length_dim.size - dim_seq.size, j)
                mask = mtf.cast(mask, tf.float32) * -1e10
                dots += mask

                attn = mtf.softmax(dots, memory_length_dim)
                out = mtf.einsum([attn, v], [dim_batch, dim_heads, dim_seq, dim_kv])

                out = mtf.transpose(out, [dim_batch, dim_seq, dim_heads, dim_kv])
                out = mtf.reshape(out, [dim_batch, dim_seq, dim_embd])
        ...
        out = linear(out, 'o', dim_embd, params=params)
...
def block(x, params, scope, past, layer_num, bias, memory_length_dim, train=False, context=None):
...
def model(mtf_features, other_features, params, mesh, past=None, context=None):
    ...
    for layer, past in enumerate(pasts):
        # attn blocks
        h, loss = block(h, params=params, scope='h%d' % layer, past=past, layer_num=layer,
                         bias=other_features["attn_bias"],
                         memory_length_dim=other_features["memory_length_dim"], context=context)
