...
CUSTOM_FNS = {'lrelu001': lambda x: mtf.leaky_relu(x, alpha=0.01),
              ...
              'elish': _elish,
              'arcsinh': _arcsinh,
              'aria': lambda x: x * (_var(x, 0) + _var(x, 1) / (_pos_var(x, 0) + _var(x, 1) * mtf.exp(_var(x, -1) * x) ** (1 / _pos_var(x, 1)))),
...
def get_activation_fn(params):
    ...
    if fn_name in BASE_FNS:
        return BASE_FNS[fn_name]

    if fn_name in CUSTOM_FNS:
        return CUSTOM_FNS[fn_name]
