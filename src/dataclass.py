...
class ModelParameter(dict):
    def __init__(self, config=None, **config_kwargs):
        ...
        self.model_path = (
            "gs://text-datasets/video-transformer/ctx=32-layer=64-heads=8-feat=256"
        )
        ...
        self.channel_color_size = (
            self.color_channels * self.time_patch * self.patch_size ** 2
        )
        ...
        self.intermediate = [
            mtf.Dimension(
                "_intermediate",
                int(
                    np.prod([dim.size for dim in self.feature_dims])
        ...
        self.token_patch_count = (
            self.language_token_per_frame // self.token_patch_size * self.use_language
        )
    ...
    def _linear(
        ...
        old: typing.List[mtf.Dimension],
        new: typing.List[mtf.Dimension],
    ) -> mtf.Tensor:
        with tf.variable_scope(random_name()):
            return mtf.einsum(
                [block_input, self._orthogonal_var(old + new)],
                deduplicate((block_input.shape - old).dims + new),
            )

    def _linear_to_features(
        self,
        block_input: mtf.Tensor,
        old: typing.Optional[typing.List[mtf.Dimension]] = None,
    ) -> mtf.Tensor:
        return self._linear(
            block_input, default(old, self.intermediate), self.feature_dims
        )

    def _linear_from_features(
        self,
        block_input: mtf.Tensor,
        new: typing.Optional[typing.List[mtf.Dimension]] = None,
    ) -> mtf.Tensor:
        return self._linear(
            block_input, self.feature_dims, default(new, self.intermediate)
        )

    def _block_fn(self, block_input) -> mtf.Tensor:
        self._layer_idx += 1

        attention_dims = (block_input.shape - self.feature_dims)[
            1:
        ...
        with tf.variable_scope(random_name()):
            ...
            context_qry = self._linear_to_features(base) + self._embed(
                [dim] + self.feature_dims
            )
            ...
            context_logits = (
                mtf.einsum([context_qry, context_key],
                           reduced_dims=[self.key_dim])
                * attention_scale
            )
            feature_logits = (
                mtf.einsum([feature_qry, learned_key],
                           reduced_dims=[self.key_dim])
                * attention_scale
            )
            ...
            max_logits = mtf.maximum(
                mtf.reduce_max(mtf.stop_gradient(
                    context_logits), reduced_dim=tmp_dim),
                mtf.reduce_max(
                    mtf.stop_gradient(feature_logits), reduced_dim=self.learned_dim[0]
                ),
            )
            context_logits = mtf.exp(context_logits - max_logits)
            feature_logits = mtf.exp(feature_logits - max_logits)
            sumexp = mtf.reduce_sum(
                context_logits, reduced_dim=tmp_dim
            ) + mtf.reduce_sum(feature_logits, reduced_dim=self.learned_dim[0])
            output = mtf.einsum(
                [context_logits / sumexp, context_val], context_qry.shape
    ...
    def build(
        ...
        spatial_ctx = (
            txt_tgt.shape[-self.feature_dim_count]
        ...
        if self.use_video:
            ...
            src = slice(
                vid, 0, context_dimension.size - 1, context_dimension
            ) / self._scalar(127.5) + self._scalar(-1)
            src = self._linear_to_features(src, input_features)

        if self.use_language:
            txt_src = self._linear_to_features(
                mtf.one_hot(txt_src, self.vocab_dim, dtype=self.dtype), [
                    self.vocab_dim]
            )
        ...
        for pad, (name, _) in zip(
            self.memory_token, src.shape[1: -self.feature_dim_count]
        ):
            if pad > 0:
                anonymous_name = "_" + name
                memory = mtf.broadcast(
                    self._embed(
                        [mtf.Dimension(anonymous_name, pad)] +
                        self.feature_dims
                    ),
                    [
                        mtf.Dimension(anonymous_name,
                                      pad) if d.name == name else d
                        for d in src.shape
                    ],
                )
        ...
        for pad, (name, size) in zip(
            self.memory_token, src.shape[1: -self.feature_dim_count]
        ):
            out = slice(out, pad, size, name)

        if self.use_language:
            tkn = self._linear_from_features(
                slice(out, 0, self.token_patch_count, spatial_ctx),
            ...
            tkn_loss = mtf.reduce_sum(
                tkn
                * (
                    self.label_smoothing / self.vocab_size /
                    (1 - self.label_smoothing)
                    + mtf.one_hot(txt_tgt, self.vocab_dim, dtype=tkn.dtype)
                )
            )
            tkn_loss *= 1 - self.label_smoothing
            token_loss: mtf.Tensor = mtf.add_n([z_loss, logsumexp, tkn_loss]) / (
