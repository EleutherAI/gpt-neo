...
class ModelParameter(dict):
    ...
    def _block_fn(self, block_input) -> typing.Tuple[mtf.Tensor, mtf.Tensor]:
        ...
        with tf.variable_scope(random_name()):
            weights = mtf.softmax(self._linear_from_features(block_input, [self.head_dim, self.anonymous_head_dim]),
                                  self.anonymous_head_dim)
            ...
            base = activate(self._linear_from_features(mtf.einsum([block_input, one_hot, weights], block_input.shape)))
