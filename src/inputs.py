...
def tf_record_dataset(
    ...
    interleave_func: object,
):
    data = tf.data.TFRecordDataset(
        filenames=tf.convert_to_tensor([name]),
        buffer_size=2 ** 26,
    ...
    data = data.window(
        size=sequence_length + time_delay,
        stride=1,
        shift=sequence_length,
        drop_remainder=True,
    )
    data = data.interleave(
        interleave_func, cycle_length=1, num_parallel_calls=1, block_length=1
    )
...
def generic_data(params: ModelParameter):
    ...
    assert not (
        language_token_per_frame > 0 and time_patch > 1
    ), "Time patch and language token are currently not supported together"

    if params.use_video:
        frame_decoder = get_decoder(
        ...
        def frame_decoder(x):
            return tf.sparse.to_dense(
                tf.parse_single_example(
                    x, {"text": tf.VarLenFeature(tf.int64)})["text"]
            )
    ...
    if params.use_video and params.language_token_per_frame > 0:
        ...
        def interleave_func(x, y, z):
            return tf.data.Dataset.zip((x, y, z)).batch(
                sequence_length + time_patch, drop_remainder=True
            )

    elif params.use_video:

        def interleave_func(x):
            return x.batch(sequence_length + time_patch, drop_remainder=True)

    else:

        def interleave_func(x):
            return x.batch(sequence_length // 1024, drop_remainder=True)

    path = [
        f"gs://{bucket_name}/{itm.name}"
    ...
    data = data.interleave(
        lambda x: tf_record_dataset(
            x, sequence_length, time_patch, frame_decoder, interleave_func
        ),
        ...
        if params.use_language:
            token = tf.reshape(
                ...
                (
                    batch_size,
                    sequence_length
                    + time_patch * params.use_video
                    + (sequence_length // 1024) * (1 - params.use_video),
                )
